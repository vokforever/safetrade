...
class SafeTradeClient:
    ...
    async def get_current_price(self, market_symbol: str) -> float | None:
        """Получает последнюю цену сделки (last) из тикера."""
        await self.init()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict):
                    # ИСПОЛЬЗУЕМ 'last' ВМЕСТО 'bid', ТАК КАК API НЕ ОТДАЕТ BID/ASK
                    if 'last' in data:
                        price = float(data.get('last', 0.0))
                    # Резервная проверка, если структура вдруг изменится
...
async def scheduled_sell_task():
    ...
    try:
        ...
        if balance is None or balance <= MIN_SELL_AMOUNT:
            logger.info(f"🗓️ Задача завершена: баланс {CURRENCY_TO_SELL} ({balance or 0}) недостаточен.")
            return

        logger.info(f"🗓️ Баланс {balance} {CURRENCY_TO_SELL} достаточен. Получаю цену...")
        price = await safetrade_client.get_current_price(MARKET_SYMBOL) # ИЗМЕНЕНО
        if not price:
